...
```
    ...
    // ── Power-up state ───────────────────────────────────────────────
    this.shieldActive    = false;   // blocks next wrong answer
    this.freezeActive    = false;   // timer is paused
    this.skipUsedThis    = false;   // skip used on current question
...
_rebuildGameCard() {
    this.gameCard.innerHTML = `
        ...
        <div class="powerup-bar" id="powerupBar">
            <button id="pu_btn_shield" class="pu-btn" onclick="game.usePowerup('shield')" title="Shield – blocks one wrong answer">🛡️ <span id="pu_count_shield">×0</span></button>
            <button id="pu_btn_freeze" class="pu-btn" onclick="game.usePowerup('freeze')" title="Freeze – pause timer 10s">⏸️ <span id="pu_count_freeze">×0</span></button>
            <button id="pu_btn_skip"   class="pu-btn" onclick="game.usePowerup('skip')"   title="Skip – skip without penalty">⏭️ <span id="pu_count_skip">×0</span></button>
            <button id="pu_btn_retry_stars" class="pu-btn" onclick="game.usePowerup('retry_stars')" title="Retry Keep Stars – retry without losing stars">🔄 <span id="pu_count_retry_stars">×0</span></button>
        </div>
...
checkAnswer(choice) {
    ...
    if (choice === this.correctAnswerPosition) {
        ...
        if (this.shieldActive) {
            // Shield absorbs the wrong answer — no penalty, no score loss
            this.shieldActive = false;
            buttons[choice].classList.add('wrong');
            buttons[this.correctAnswerPosition].classList.add('correct');
            this._playSound('correct');
            this._showAchievement('🛡️ Shield blocked that wrong answer!');
            this._refreshPowerupBar();
        } else {
            buttons[choice].classList.add('wrong');
            buttons[this.correctAnswerPosition].classList.add('correct');
...
// ── Power-up inventory helpers ──────────────────────────────────────
_getPowerupInventory() {
    return JSON.parse(localStorage.getItem('mathQuest_powerups') || '{}');
}
_savePowerupInventory(inv) {
    localStorage.setItem('mathQuest_powerups', JSON.stringify(inv));
}
_getPowerupCount(id) {
    return this._getPowerupInventory()[id] || 0;
}
_consumePowerup(id) {
    const inv = this._getPowerupInventory();
    if ((inv[id] || 0) <= 0) return false;
    inv[id]--;
    if (inv[id] <= 0) delete inv[id];
    this._savePowerupInventory(inv);
    this._refreshPowerupBar();
    return true;
}

// Render the power-up bar counts
_refreshPowerupBar() {
    const ids = ['shield','freeze','skip','retry_stars'];
    ids.forEach(id => {
        const el = document.getElementById(`pu_count_${id}`);
        if (el) {
            const n = this._getPowerupCount(id);
            el.textContent = `×${n}`;
            const btn = document.getElementById(`pu_btn_${id}`);
            if (btn) btn.disabled = (n <= 0);
        }
    });
    // Highlight active ones
    const shieldBtn = document.getElementById('pu_btn_shield');
    if (shieldBtn) shieldBtn.classList.toggle('pu-active', this.shieldActive);
    const freezeBtn = document.getElementById('pu_btn_freeze');
    if (freezeBtn) freezeBtn.classList.toggle('pu-active', this.freezeActive);
}

// ── Power-up actions (called from buttons) ─────────────────────────
usePowerup(id) {
    if (this.levelCompleted) return;
    switch(id) {
        case 'shield':       this._puShield();      break;
        case 'freeze':       this._puFreeze();      break;
        case 'skip':         this._puSkip();        break;
        case 'retry_stars':  this._puRetryStars();  break;
    }
}

_puShield() {
    if (this.shieldActive) { this._showAchievement('🛡️ Shield already active!'); return; }
    if (this._getPowerupCount('shield') <= 0) { this._showAchievement('🛡️ No Shields left! Buy in Shop.'); return; }
    this._consumePowerup('shield');
    this.shieldActive = true;
    this._showAchievement('🛡️ Shield activated! Next wrong answer blocked.');
    this._refreshPowerupBar();
}

_puFreeze() {
    if (this.freezeActive) { this._showAchievement('⏸️ Timer already frozen!'); return; }
    if (this._getPowerupCount('freeze') <= 0) { this._showAchievement('⏸️ No Freeze left! Buy in Shop.'); return; }
    this._consumePowerup('freeze');
    this.freezeActive = true;
    clearInterval(this.timerInterval);
    this._showAchievement('⏸️ Timer frozen for 10 seconds!');
    this._refreshPowerupBar();
    setTimeout(() => {
        this.freezeActive = false;
        if (!this.levelCompleted) this._startTimer();
        this._showAchievement('▶️ Timer resumed!');
        this._refreshPowerupBar();
    }, 10000);
}

_puSkip() {
    if (this._getPowerupCount('skip') <= 0) { this._showAchievement('⏭️ No Skips left! Buy in Shop.'); return; }
    if (this.skipUsedThis) { this._showAchievement('⏭️ Already skipped this question!'); return; }
    this._consumePowerup('skip');
    this.skipUsedThis = true;
    this._showAchievement('⏭️ Question skipped — no penalty!');
    // Advance without counting as wrong
    this.questionCount++;
    this._updateProgress();
    if (this.questionCount < this.questionsPerLevel) {
        this.secondsLeft = this.timePerQuestion;
...
_puRetryStars() {
    if (this._getPowerupCount('retry_stars') <= 0) { this._showAchievement('🔄 No Retry tokens left! Buy in Shop.'); return; }
    this._consumePowerup('retry_stars');
    this._showAchievement('🔄 Retrying — your stars are safe!');
    // Restart level but preserve stars already earned
    const savedStars = this.starsEarned;
    setTimeout(() => {
        this.startLevel(this.currentLevel);
        this.starsEarned = savedStars; // keep stars
    }, 800);
}
